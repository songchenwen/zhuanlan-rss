...
var retryWithCache = function(id, cb){
	console.log('cache retry for ' + id);
	storage.get([id], function(err, results){
		if(err){
			console.log('retry storage ' + id + ' error ' + err);
			cb();
			return;
		}
		if(!results || results.length == 0){
			console.log('retry not found ' + id);
			cb();
			return;
		}
		var list = results[0];
		...
		console.log('got ' + list.length + ' items of ' + id + ' from retry storage, maybe outdated');
		list.forEach(function(i){
			items.push(i);
		});
		cb();
