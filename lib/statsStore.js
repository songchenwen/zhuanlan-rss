...
Store.prototype.popularZhuanlan = function(options, cb){
	...
	db.find({type: typeZhuanlan, date:{ $lte:to, $gt:from }}, function(err, docs){
		...
		async.each(results, function(z, c){
			db.find({type:typeItem, zhuanlan:z.zhuanlan, date:{ $lte:to, $gt:from }}, function(err, docs){
				if(err){
					z.itemIpCount = 0;
					c(err);
					return;
			...
			results.sort(function(a, b){
				if(b.itemIpCount == a.itemIpCount){
				...
				return b.itemIpCount - a.itemIpCount;
			});
			if(results.length > limit){
				results = results.slice(0, limit);
			}
